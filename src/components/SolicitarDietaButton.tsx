...
import { Utensils, Loader2, CheckCircle2 } from 'lucide-react'

export function SolicitarDietaButton() {
  ...
  async function solicitar() {
    ...
    if (!res.ok) {
      setMsg(data.error ?? 'Erro ao solicitar dieta.')
      setState('error')
    } else {
      setMsg('Solicitação enviada! Aguarde seu nutricionista criar o plano.')
      setState('done')
    }
  }
    ...
    <div className="space-y-3">
      ...
      >
        {state === 'loading' ? (
          ...
          <><Utensils className="w-4 h-4" /> Solicitar plano de dieta</>
        )}
      </button>
      {state === 'error' && msg && (
