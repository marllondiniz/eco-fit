...
export default function EditarTreinoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: treino }, { data: clientesData }] = await Promise.all([
        supabase
          .from('workouts')
      ...
      setNome(treino.name ?? '')
      setDivisao(treino.division ?? '')
      setMetodologia(treino.methodology ?? '')
      setObservacoes(treino.notes ?? '')
      setClienteId(treino.client_id ?? '')
      setClientes((clientesData as Cliente[]) ?? [])

      const exs = (treino.workout_exercises ?? [])
      ...
      setExercicios(exs.length ? exs : [emptyExercise()])
      setLoading(false)
    }
    load()
  }, [id, router])
  ...
  async function gerarComIA() {
    ...
    try {
      ...
      if (data.divisao) setDivisao(data.divisao)
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold text-foreground">Editar Treino</h2>
          <p className="text-muted-foreground mt-1 text-sm">Atualize o plano de treino.</p>
        </div>
        <Button
          type="button"
          onClick={gerarComIA}
        ...
        <CardContent className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Select value={clienteId || 'none'} onValueChange={v => setClienteId(v === 'none' ? '' : v)}>
              <SelectTrigger id="cliente"><SelectValue placeholder="Selecionar cliente (opcional)" /></SelectTrigger>
              <SelectContent>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input id="nome" value={nome} onChange={e => setNome(e.target.value)} placeholder="Ex: Hipertrofia A/B/C" />
            </div>
            <div className="space-y-1.5">
              ...
              <Select value={objetivo} onValueChange={setObjetivo}>
                ...
                <SelectContent>
                  {OBJETIVOS_TREINO.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="divisao" value={divisao} onChange={e => setDivisao(e.target.value)} placeholder="Ex: A/B/C" list="divisoes" />
            <datalist id="divisoes">{DIVISOES_SUGERIDAS.map(d => <option key={d} value={d} />)}</datalist>
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="metodologia">Metodologia aplicada</Label>
            <Textarea id="metodologia" value={metodologia} onChange={e => setMetodologia(e.target.value)} rows={3} placeholder="Metodologia de treino..." />
          </div>
          <div className="space-y-1.5">
                ...
                <div className="flex-1 space-y-3">
                  <div className="grid sm:grid-cols-3 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input value={exercicio.division_label} onChange={e => updateExercicio(idx, 'division_label', e.target.value)} placeholder="A, B, C..." list="divisoes-label" className="text-sm" />
                      <datalist id="divisoes-label">{DIVISOES_SUGERIDAS.map(d => <option key={d} value={d} />)}</datalist>
                    </div>
                    <div className="sm:col-span-2 space-y-1.5">
                      <Label className="text-xs">Nome do exercício *</Label>
                      <Input value={exercicio.name} onChange={e => updateExercicio(idx, 'name', e.target.value)} placeholder="Ex: Supino reto com barra" className="text-sm" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input value={exercicio.sets} onChange={e => updateExercicio(idx, 'sets', e.target.value)} placeholder="4" type="number" className="text-sm" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs">Repetições</Label>
                      <Input value={exercicio.reps} onChange={e => updateExercicio(idx, 'reps', e.target.value)} placeholder="8–12" className="text-sm" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs">Descanso (seg)</Label>
                      <Input value={exercicio.rest_seconds} onChange={e => updateExercicio(idx, 'rest_seconds', e.target.value)} placeholder="60" type="number" className="text-sm" />
                    </div>
                  </div>

                  <div className="space-y-1.5">
                    <Label className="text-xs">Observações</Label>
                    <Input value={exercicio.notes} onChange={e => updateExercicio(idx, 'notes', e.target.value)} placeholder="Técnica, cadência, variações..." className="text-sm" />
                  </div>
                </div>
                {exercicios.length > 1 && (
                  <button onClick={() => removeExercicio(idx)} className="mt-6 p-1.5 rounded-lg text-muted-foreground hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors flex-shrink-0">
